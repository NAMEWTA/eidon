...
| 限定写法 | 含义 | 范围 |
|---|---|---|
| **三层【代码】** | `src(React UI) → core(TS 业务核心) → src-tauri(Rust 能力壳)`，软件分层、单向依赖、`core/bridge/` 唯一 Tauri 出口、ESLint 机器强制 | 工程架构（AGENTS.md §2.1 / 工程层 ADR-0006/0007）。**本期复用不动其架构。** |
| **三层【节点】** | `L1 / L2 / L3`，数据节点层级，深度=层级铁律，第 4 层起为自由文件夹（无身份） | 产品数据层（产品层 ADR-002 / 工程层 ADR-0013）。**本期要造的新地基。** |
...
| 语境 | 含义 |
|---|---|
| 版本 / 时光机（EIDON 用） | 普通编辑的版本历史 = 现有 git 的 auto-commit + `history.commitNow` + HistoryPanel（`git_history.rs`）。**autoGit 无需修改、不另做快照功能，diff/恢复直接用 git**；`core/snapshots` 仅薄封装现有 git bridge（工程层 ADR-0015） |
...
- **ULID 身份**：节点硬 ID，复用 `core/shared/id`，与路径无关、随目录走、可遍历重建。
- **可重建铁律（AX-1/AX-4）**：删 `.eidon/` 索引缓存后，遍历 `.node/` + `.eidon/templates/` 可 100% 重建节点树/身份/字段/id→path。

---

## 3. `core/` 数据层四模块（工程层 ADR-0012）

> EIDON 数据层按**细模块切分**落在 `core/`，为可扩展性 + 可读性新增**四个**并列业务模块（取代早期「只加两个」）。各自 `index.ts`、禁 UI 框架、不互 import 内部。

| 模块 | 本期职责 | 后续深化 |
|---|---|---|
| **`core/nodes`** | ULID 身份 / node.json 读写 / 扫描建树（深度=层级 + id→path）/ 创建·重命名·移动·提升为节点 | — |
| **`core/templates`** | 6 类字段 schema 版本化 / 编辑生成新版 / 删除孤儿模板态 / 内置种子 / 给设置内模板管理 UI 供数据 | 跨模板字段复用 |
| **`core/snapshots`** | 版本能力归属：**直接复用现有 git（autoGit 不改、不做快照功能）**，仅薄封装 git bridge（ADR-0015） | 如未来确需再另立 |
| **`core/consistency`** | 结构违规**检测 + 标记**（四类，ADR-0016） | 软态身份系统 / 自动补全 / 一致性面板 |
...
- **结构违规标记（FileTree 标记）**：`core/consistency` 扫描检测到不符合结构的东西（外部产生/历史扁平库），**只在 FileTree 打徽标**、不自动处理、无独立面板。四类：①前三层普通文件夹「待提升」②L1/L2 内容文件「位置非法/待下沉」③节点深度与 level 不符「层级不符」④`.node/` 缺失「结构待修复」。
...
- **一致性软态体系（后续）**：越界 `flags.outOfPlace` / 孤儿 `flags.orphan` / 孤儿模板 `flags.orphanTemplate` / 失联 disconnected + 独立一致性面板 + 自动补全 + 自动整理开关 +「其他」L3 兜底节点。本期 `core/consistency` 只做检测+标记。
...
- **工程层 ADR** = `speculo/.speculo/.config/adr/ADR-0001~0018`（见该目录 `README.md` 登记表）。约束代码架构与 EIDON 2.0 数据层落点。引用写 `工程层 ADR-00XX`。
  - 工程基座：0001/0005/0006/0007/0009/0010；EIDON 2.0 数据层：0011~0018。（旧 AI 子系统 ADR 0002/0003/0004/0008 已随重构删除，编号留空不复用。）
...
- 两套并存、不冲突；**引用时必须标注来源轨道**，避免编号混淆（如「产品层 ADR-002」vs「工程层 ADR-0002」）。
