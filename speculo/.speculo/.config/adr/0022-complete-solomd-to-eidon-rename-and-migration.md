...
事件名、DOM 全局 / CSS 类 / 环境变量、工作区目录、OS 钥匙串服务名、
...
| 运行时事件 | `solomd:*` → `eidon:*`（Electron IPC `webContents.send` + 前端 `bridge/ipc/events.listen` 同改） |
| DOM 全局 / CSS / sentinel / env | `__solomd*`→`__eidon*`、`.solomd-print*`→`.eidon-print*`、`solomd-custom-theme`→`eidon-custom-theme`、`__solomd_truncated__`（跨层）、`SOLOMD_APP_STORE_BUILD`→`EIDON_APP_STORE_BUILD`、`"SoloMD CJK"`→`"EIDON CJK"`（字体名） |
...
- **远程主题市场**：删孤儿 `stores/themes.ts` + `editor/themes.rs`（`theme_install`/`uninstall`/`list_installed`）。**内置主题预设 + 自定义 CSS 主题保留不动。**
...
- 代码内 `solomd` 残留**归零**，仅剩 `backend/domain/ai.ts` 中 `原 SoloMD …` 历史性说明（描述已删 AI 子系统的来历，属 ADR-0019 范畴，保留）。
