...
EIDON 是完整重构，**不背任何旧数据迁移 / 向后兼容**包袱（呼应 ADR-0011）。磁盘上跨语言 / 跨消费方共享的结构（TS 生产、Rust 读取、扫描重建）用**契约 + 共享 golden fixtures** 防漂移：契约以 `core/contracts/*.ts`（zod）为单一事实源，`fixtures/contracts/` 提供语言中立 golden 样本，TS 与 Rust 两侧测试加载同一组 fixtures 做 conformance。
