...
- **单一生成器**：`scripts/generate-brand-icon.mjs` 程序化产出全套品牌资产——应用图标多尺寸
  + `app/resources/eidon-pet-sheet.png` 精灵表（行=形态、列=帧）+ idle 动图归档。资产由代码生成、可复现，不手绘散落。
- **三处耦合（必须同步，否则精灵错位）**：
  1. 生成器 `STATE_ORDER`（精灵表行的**权威顺序**）；
  2. `app/frontend/styles/components.css` 的 `.eidon-pet--<state>` 行映射（`background-position-y`）+ `steps()` 走帧；
  3. `app/frontend/components/EidonPet.tsx` 的 `PetState`。
