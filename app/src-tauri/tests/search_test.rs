...
fn finds_needle_across_files() {
    ...
    let hits: Vec<SearchHit> =
        search_in_dir("/tmp/eidon-search-test".to_string(), "needle".to_string(), 100).unwrap();
    println!("hits: {:#?}", hits);
...
fn case_insensitive() {
    ensure_fixture();
    let hits = search_in_dir("/tmp/eidon-search-test".to_string(), "NEEDLE".to_string(), 100).unwrap();
    assert!(hits.len() >= 3);
}
...
fn empty_query_returns_empty() {
    ensure_fixture();
    let hits = search_in_dir("/tmp/eidon-search-test".to_string(), "".to_string(), 100).unwrap();
...
fn respects_max_results() {
    ensure_fixture();
    let hits = search_in_dir("/tmp/eidon-search-test".to_string(), "the".to_string(), 2).unwrap();
    assert!(hits.len() <= 2);
}
