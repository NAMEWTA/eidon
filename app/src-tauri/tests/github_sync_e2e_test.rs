...
fn fresh_dir(label: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("eidon-ghs-{label}-{nanos}"));
...
fn init_workspace_with_remote(label: &str, bare_url: &str) -> PathBuf {
    ...
    // Write the .eidon-sync/sync.json the inner helpers expect.
    let cfg_dir = ws.join(".eidon-sync");
    ...
    // Mirror what github_link_workspace does in production: keep .eidon-sync/
    // out of the tracked tree so per-device sync.json never conflicts.
    fs::write(ws.join(".gitignore"), ".eidon-sync/\n").unwrap();
    ws
}
...
fn github_sync_corrupted_config_fails_closed() {
    ...
    fs::write(dev.join(".eidon-sync/sync.json"), b"{not valid json").unwrap();
