...
fn fresh(label: &str) -> PathBuf {
    ...
    let p = std::env::temp_dir().join(format!("eidon-e2ee-{label}-{nanos}"));
...
fn write_sync_json(folder: &Path, remote_url: &str) {
    let cfg_dir = folder.join(".eidon-sync");
    ...
    fs::write(folder.join(".gitignore"), ".eidon-sync/\n.eidon-encrypted/\n").unwrap();
}

fn init_shadow_with_remote(folder: &Path, remote_url: &str) {
    let shadow = folder.join(".eidon-encrypted");
...
fn e2ee_full_round_trip_two_devices() {
    ...
    // libgit2's default-branch handling at clone time.
    {
        ...
        assert_eq!(&enc_blob.content()[..4], b"EIDN", "ciphertext magic missing");
    ...
    assert!(
        dev_b.join(".eidon-encrypted").join(".eidon-vault.json").exists(),
    ...
    assert!(shadow.ends_with(".eidon-encrypted"));
}
