use app_lib::file_ops::{read_file_inner as read_file, write_file_inner as write_file};
use std::fs;

#[test]
fn read_utf8_with_bom() {
    let path = "/tmp/eidon-utf8-bom.md".to_string();
...
#[test]
fn read_utf8_no_bom() {
    let path = "/tmp/eidon-utf8.md".to_string();
...
fn read_gbk_chinese() {
    use encoding_rs::GBK;
    let path = "/tmp/eidon-gbk.txt".to_string();
...
fn read_utf16_le_bom() {
    let path = "/tmp/eidon-utf16le.md".to_string();
...
fn write_then_read_utf8_roundtrip() {
    let path = "/tmp/eidon-roundtrip.md".to_string();
...
fn write_gbk_roundtrip() {
    let path = "/tmp/eidon-gbk-rt.txt".to_string();
...
fn detect_language_from_extension() {
    let path = "/tmp/eidon-lang.markdown".to_string();
    fs::write(&path, "# heading").unwrap();
    let r = read_file(path).unwrap();
    assert_eq!(r.language, "markdown");

    let path2 = "/tmp/eidon-lang.txt".to_string();
    fs::write(&path2, "plain").unwrap();
    let r2 = read_file(path2).unwrap();
    assert_eq!(r2.language, "plaintext");

    let path3 = "/tmp/eidon-lang.rs".to_string();
