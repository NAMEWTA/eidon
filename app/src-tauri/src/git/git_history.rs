...
/// the user's global git config; falls back to `eidon@local`.
...
pub fn git_init_workspace_inner(
    ...
    let msg = initial_message.unwrap_or_else(|| "init: EIDON workspace".to_string());
...
pub async fn git_auto_commit(
    ...
    tauri::async_runtime::spawn_blocking(move || git_auto_commit_inner(folder, file_path, message))
        .await
        .map_err(|e| format!("join: {e}"))?
}
