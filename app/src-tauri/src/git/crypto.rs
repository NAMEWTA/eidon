...
//! continue to work).
//!
//! Architecture: the workspace owns a `.eidon-encrypted/` shadow
...
//! `.eidon-sync/encryption.json`. The derived 32-byte key lives in the OS
//! keychain (`eidon-encryption-key`); the passphrase itself is never
//! persisted.
//!
//! Per-file format: ciphertext begins with a 4-byte magic ("EIDN"), 1-byte
...
const KEYRING_SERVICE: &str = "eidon-encryption-key";
const ENCRYPTION_CONFIG_FILE: &str = ".eidon-sync/encryption.json";
const SHADOW_DIR: &str = ".eidon-encrypted";
...
const SHADOW_SALT_FILE: &str = ".eidon-vault.json";
const FILE_MAGIC: &[u8; 4] = b"EIDN";
...
pub struct EncryptionConfig {
    ...
    pub salt: String,
    /// Argon2id parameters — captured so a future EIDON with bumped
...
pub fn encrypt_bytes(key: &[u8; 32], aad_path: &str, plaintext: &[u8]) -> Result<Vec<u8>, String> {
    ...
    hasher.update(key);
    hasher.update(b"\x00eidon-nonce-v1\x00");
...
pub fn decrypt_bytes(key: &[u8; 32], aad_path: &str, blob: &[u8]) -> Result<Vec<u8>, String> {
    ...
    if &blob[..4] != FILE_MAGIC {
        return Err("bad magic — not an EIDON ciphertext".into());
    }
...
fn key_marker_path(workspace: &Path) -> PathBuf {
    workspace.join(".eidon-sync/encryption.key-set")
}
...
pub fn crypto_set_passphrase(folder: String, passphrase: String) -> Result<(), String> {
    ...
    // blob. The probe lives in `.eidon-sync/encryption.probe.enc`; if it
    // round-trips we accept the passphrase, otherwise we refuse rather
    // than silently overwrite the keyring with a wrong key.
    let probe_path = path.join(".eidon-sync/encryption.probe.enc");
    ...
    if fresh {
        let probe = encrypt_bytes(&key, "encryption.probe", b"EIDON probe v1")?;
        fs::write(path.join(".eidon-sync/encryption.probe.enc"), probe)
            .map_err(|e| e.to_string())?;
    }
    Ok(())
}
...
/// vault's `encrypt_extensions` list, and write to `<workspace>/.eidon-encrypted/`.
...
pub fn crypto_encrypt_for_push_inner(folder: String) -> Result<String, String> {
    ...
    for entry in WalkDir::new(&path).into_iter().filter_map(|e| e.ok()) {
        ...
        if p.starts_with(path.join(".eidon-sync")) {
...
/// Inverse of `crypto_encrypt_for_push`. Walks `<workspace>/.eidon-encrypted/`,
...
mod tests {
    ...
    fn fresh(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("eidon-crypto-{label}-{nanos}"));
    ...
    fn workspace_round_trip() {
        ...
        assert!(!shadow_dir.join(".eidon-sync/sync.json").exists());
