...
//!   * `github_link_workspace` — write `.eidon-sync/sync.json`, set remote
...
const KEYRING_SERVICE: &str = "eidon-github";
const KEYRING_USER: &str = "personal-access-token";
const SYNC_CONFIG_FILE: &str = ".eidon-sync/sync.json";
const GITHUB_API: &str = "https://api.github.com";
const USER_AGENT: &str = "EIDON-sync/2.6";
...
/// `~/.eidon-sync/github-token-set` — a non-secret marker file written
...
fn token_marker_path() -> Option<std::path::PathBuf> {
    std::env::var_os("HOME")
        ...
        .map(|h| h.join(".eidon-sync").join("github-token-set"))
}
...
pub async fn github_create_vault_repo(name: String, private: bool) -> Result<GitHubRepo, String> {
    ...
    let req = CreateRepoRequest {
        ...
        auto_init: true,
        description: "Notes vault — synced by EIDON",
    };
...
pub struct SyncConfig {
    ...
    /// v2.6.3 — when true, push/pull operate on `<workspace>/.eidon-encrypted/`
...
const ENCRYPTED_SHADOW: &str = ".eidon-encrypted";
...
/// Ensure `.eidon-sync/` is in the workspace's git ignore list. Each device
...
fn ensure_gitignore_excludes_eidon_state(workspace: &Path) -> Result<(), String> {
    let gitignore = workspace.join(".gitignore");
    let existing = fs::read_to_string(&gitignore).unwrap_or_default();
    if existing
        .lines()
        .any(|l| l.trim() == ".eidon-sync/" || l.trim() == ".eidon-sync")
    {
        return Ok(());
    }
    ...
    new.push_str("# EIDON workspace metadata — per-device, do not sync.\n");
    new.push_str(".eidon-sync/\n");
...
pub async fn github_link_workspace(
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<SyncConfig, String> {
        ...
        // .eidon-sync/ holds per-device state — never committed.
        // .eidon-encrypted/ is the shadow git repo when E2EE is on —
        ...
        ensure_gitignore_excludes_eidon_state(&path)?;
        Ok(cfg)
    })
    .await
    .map_err(|e| e.to_string())?
...
/// 2. Initialises the encrypted shadow at `<workspace>/.eidon-encrypted/`
...
pub async fn github_enable_encryption(
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<(), String> {
        ...
        // Local .gitignore inside the shadow — the shadow's own `.eidon-sync/`
...
// Storage: ~/.eidon-sync/proxy (single-line URL like
...
fn proxy_path() -> Option<PathBuf> {
    std::env::var_os("HOME")
        ...
        .map(|h| h.join(".eidon-sync").join("proxy"))
}
...
pub fn github_push_inner(folder: String, token: String) -> Result<(), String> {
    ...
    // different machine (or pre-v3.0 EIDON) can land here still on
...
pub fn github_pull_inner(folder: String, token: String) -> Result<PullResult, String> {
    ...
    if cfg.encrypted {
        ...
        if analysis.0.is_fast_forward() {
            ...
            head_ref
                .set_target(upstream_oid, "fast-forward via EIDON GitHub sync")
                .map_err(|e| e.to_string())?;
...
fn signature(repo: &Repository) -> Result<Signature<'static>, String> {
    ...
    Signature::now("EIDON", "eidon@local").map_err(|e| e.to_string())
}
