...
//!     `EIDON_APP_STORE_BUILD=1` before invoking `pnpm tauri build`.
...
pub const IS_APP_STORE: bool = option_env!("EIDON_APP_STORE_BUILD").is_some();
...
pub fn app_store_disabled<T>() -> Result<T, String> {
    Err("This feature is not available in the App Store edition. Download the GitHub release for the full EIDON experience.".to_string())
}
