...
//! the EIDON WebView, and returns whatever the script resolves to. This is
//! how `eidon-dev-mcp` drives the actual Vue UI from outside the app.
//!
...
//! `nm app/src-tauri/target/release/EIDON | grep dev_bridge` and you
...
async fn handle_conn(
    ...
    if req.method == "GET" && req.path == "/" {
        write_resp(
            ...
            200,
            "OK",
            "{\"ok\":true,\"server\":\"eidon-dev-bridge\"}",
            true,
        )
        .await
        .ok();
...
async fn handle_eval(req: &Request, app: &AppHandle, port: u16) -> Result<JsonValue, String> {
    ...
    // application "EIDON" to activate'`) and retry. We tried dispatching
