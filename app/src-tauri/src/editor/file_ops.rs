...
/// 的目录还是有些卡顿".
pub fn list_dir_inner_with_options(path: String, include_hidden: bool) -> Result<Vec<DirEntry>, String> {
...
#[tauri::command]
pub async fn list_dir(path: String, include_hidden: Option<bool>) -> Result<Vec<DirEntry>, String> {
    let include_hidden = include_hidden.unwrap_or(false);
    tauri::async_runtime::spawn_blocking(move || list_dir_inner_with_options(path, include_hidden))
        .await
        .map_err(|e| format!("join: {e}"))?
}
