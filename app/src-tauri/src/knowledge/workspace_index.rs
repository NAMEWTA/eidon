...
//! affected file is re-scanned and an `eidon://index-updated` event is
...
fn workspace_index_init_inner(app: AppHandle, folder: String) -> Result<usize, String> {
    ...
    save_cache(&app, &root)?;
    let _ = app.emit("eidon://index-updated", &"init");
    Ok(count)
}
...
fn workspace_index_rescan_inner(app: AppHandle) -> Result<usize, String> {
    ...
    scan_into(&root)?;
    save_cache(&app, &root)?;
    let _ = app.emit("eidon://index-updated", &"rescan");
    let count = STATE.read().map_err(|e| e.to_string())?.entries.len();
    Ok(count)
}
...
fn handle_event(app: &AppHandle, event: Event) {
    ...
    std::thread::spawn(move || {
        ...
        if changed {
            ...
            let _ = app_for_thread.emit("eidon://index-updated", &"watch");
        }
    });
}
