...
const makeGateway = (): SnapshotGateway => ({
  ...
  pruneHistory: vi.fn(async () => ({ commitsBefore: 10, commitsAfter: 5, sizeAfter: 600000, gcRan: true })),
});
...
describe("core/snapshots", () => {
  it("maps EIDON snapshot API calls to the existing git gateway without adding snapshot semantics", async () => {
    ...
    await expect(diffFileSnapshot("/workspace", "/workspace/a.md", "abcdef0", gateway)).resolves.toMatchObject({ toSha: "abcdef0" });
