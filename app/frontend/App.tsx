...
import { listen, onFileDrop, setWindowTitle, type UnlistenFn } from '@bridge/ipc/platform';
import { eidonInvoke } from '@bridge/ipc';
import { Toolbar } from './components/layout/Toolbar';
import { TileRoot } from './components/editor/TileRoot';
...
import { FilePropertiesPanel } from './components/panels/FilePropertiesPanel';
...
import { SessionRestoreDialog } from './components/dialogs/SessionRestoreDialog';
import { BasesView } from './components/features/BasesView';
import { OverviewPanel } from './components/panels/OverviewPanel';
import { BASES_OPEN_EVENT, BASES_CLOSE_EVENT } from './hooks/useBasesView';
import { FileTree } from './components/navigation/FileTree';
import { SettingsPanel } from './components/panels/SettingsPanel';
import { MarkdownHelp } from './components/dialogs/MarkdownHelp';
...
export function App() {
  ...
  // ---- 挂载：生命周期 composable、所有 window 监听、store.subscribe watchers、onMounted 异步块 ----
  ...
  useEffect(() => {
    ...
    function reconcile() {
      ...
      if (s.language !== prevLang) {
        prevLang = s.language;
        eidonInvoke('shell:setMenuLanguage', { lang: s.language }).catch(() => {});
        eidonInvoke('shell:saveLanguage', { lang: s.language }).catch(() => {});
      }
      ...
      if (s.spellcheckEnabled && !spellcheckLoaded) {
        eidonInvoke('kn:spellInit', { lang: 'en_US' })
    ...
    (async () => {
      ...
      try {
        unlistenClose = await listen('eidon://close-requested', async () => {
          useTabsStore.getState().persist?.();
          useTilesStore.getState().persist();
          await eidonInvoke('shell:forceClose', {});
        });
      } catch (err) {
