...
// edge cases markdown can't express. The production CSP allows this for
...
export const md = new MarkdownIt({
  ...
  highlight: (code: string, lang: string): string => {
    // Mermaid blocks are handled after-render (processMermaid in Preview)
...
// Preview intercepts clicks and resolves through the workspace index.
...
// the 1-indexed source line. App's split-scroll uses these attributes
...
md.renderer.rules.fence = function (tokens, idx, options, env, self) {
  ...
  // Skip mermaid — Preview replaces these blocks with rendered SVGs.
