 ...
 */
import type { PdfDefaults } from './persistence/settings';
import { defaultPdfDefaults } from './persistence/settings';
...
export function buildPrintStyle(opts: ResolvedPdfOptions): string {
  ...
  const fontDecl = fontFamily.trim()
    ? `body, .preview-content, .eidon-print-content { font-family: ${quoteFontFamily(fontFamily)}, "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", sans-serif !important; }`
    : '';
  const fontSizeDecl = `body, .preview-content, .eidon-print-content { font-size: ${fontSizePt}pt !important; }`;
