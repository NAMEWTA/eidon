/**
 * Shared image URL resolution for EIDON.
 * Used by Preview, pdf-export.ts, image-export.ts, and docx-export.ts
 * to convert local image paths into URLs the webview can load or bytes for embedding.
 */

import { toAssetUrl } from '@bridge/ipc/platform';

/**
 * Normalize a filesystem path so `toAssetUrl` produces a URL the
...
export function resolveImagePath(
  ...
  if (!src) return src;
  if (/^(https?|data|blob|eidon-asset):/i.test(src)) return src;
 ...
 * Converts local paths to `eidon-asset://` URLs via toAssetUrl().
 */
export function resolveImageSrc(
  ...
  if (/^(https?|data|blob|eidon-asset):/i.test(resolved)) return resolved;
  try {
...
/**
 * Local-file `<a href=…>` URLs in rendered markdown otherwise resolve
 * against the renderer's base origin, which then bakes into exported
 * PDFs / DOCX / images as a useless `<base-origin>/foo.factoryio` link.
 *
...
export function rewriteLinkUrls(
  ...
  return rawHtml.replace(
    ...
    (_match, prefix: string, q: string, href: string) => {
      ...
      if (/^(https?|mailto|tel|sms|data|blob|eidon-asset|ftp|file):/i.test(href)) {
