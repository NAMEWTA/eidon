...
export const useTabsStore = create<PersistedState & TabsActions>()((set, get) => ({
  ...
  markSaved(id, filePath) {
    ...
    // savedContent 直接取「当前 content」原样落锚 —— goal_set_at 等保存期 frontmatter 注入
    // 已由写盘**之前**的 syncBumpUpdated 统一完成（见 hooks/useFiles.ts），此处不再
    // 二次注入：保证 savedContent ≡ 刚落盘的字节（否则下次从盘加载/重启会静默丢戳）。
    set({
      tabs: get().tabs.map((x) =>
        x.id === id ? { ...x, filePath, fileName, savedContent: x.content, language, showOutline: language === 'markdown' ? (x.showOutline ?? true) : false } : x,
      ),
    });
