/**
 * cm-live-render.ts — WYSIWYG "live edit" CM6 extension for EIDON v2.3
 *
...
/**
 * Bundle for the "live edit" editor render. Wired into Editor.tsx as the
 * rich-extensions value when `editorRender === 'live'` and the tab is
 * markdown (orthogonal to the layout `viewMode`).
 *
...
// ---------------------------------------------------------------------------
// Self-test hook (used by dev-mcp `eidon_get_editor_decorations`).
//
// We expose a tiny window-level helper that, when the editor is mounted,
// reports the current visible-range decoration counts. The webview
 ...
 * `eidon_get_editor_decorations` tool (and any future automated tests)
 * can assert on them.
 */
