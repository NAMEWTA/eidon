 ...
 * image data. Saves each image to disk (via the binary-write IPC
...
async function saveAndInsert(
  ...
  if (imageRoot && filePath) {
    ...
    const eidonDir = joinPath(t, 'eidon', sepCh);
    fullPath = joinPath(eidonDir, filename, sepCh);
  ...
  try {
    await eidonInvoke('editor:writeBinaryFile', {
      path: fullPath,
 ...
 * already have the bytes), this path comes from the webview-level
 * drag-drop event, so we copy the file via the copy-file IPC
...
export async function insertImageFromPath(
  ...
  if (imageRoot && filePath) {
    ...
    const t = await resolveTempDir(opts.tempDir);
    const eidonDir = joinPath(t, 'eidon', sepCh);
    dstPath = joinPath(eidonDir, filename, sepCh);
