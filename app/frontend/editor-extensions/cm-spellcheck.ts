 ...
 *
 * Walks the visible viewport, splits each line into Latin word runs in the backend
 ...
 *
 * Performance:
 *   - Only the lines currently in the viewport are sent to the backend.
 ...
 *     edited lines round-trip to the backend.
...
interface Misspelling {
  word: string;
  start: number; // byte offsets within the *line* text we sent to the backend
  end: number;
}
...
/**
 * Convert a byte offset (returned by the backend regex over UTF-8 bytes) to a
 * UTF-16 code-unit offset (what CodeMirror works in).
 *
 * This matters because the doc may contain multi-byte chars (CJK, emoji).
 * The backend only flags Latin runs, but we still receive byte offsets,
  ...
  const plugin = ViewPlugin.fromClass(
    class {
      ...
      private async recompute() {
        ...
        // Per visible line: use cache if text matches, else invoke the backend.
        ...
        for (const lineNo of visibleLines) {
          ...
          if (cached) {
            ...
            try {
              const raw = await eidonInvoke('kn:spellCheck', { text });
