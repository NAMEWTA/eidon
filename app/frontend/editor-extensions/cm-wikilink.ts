 ...
 *     workspace index store and emits `eidon:wiki-open` (the App shell listens and
 *     opens the file).
...
function isResolved(target: string): boolean {
  ...
  try {
    const idx = useWorkspaceIndexStore.getState();
...
const wikilinkPlugin = ViewPlugin.fromClass(WikilinkPluginValue, {
  ...
  eventHandlers: {
    mousedown(e: MouseEvent) {
      ...
      e.preventDefault();
      // Defer resolution to the App shell (it has access to file actions).
      window.dispatchEvent(new CustomEvent('eidon:wiki-open', { detail: { target: inner } }));
      return true;
    },
  },
});
 ...
 * Editor with the other markdown autocompletion sources, since CM6
