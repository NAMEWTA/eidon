...
// back to a "rendering…" placeholder, then dispatches `eidon:cm-relayout`
  ...
  toDOM(): HTMLElement {
    ...
    if (cached?.svg) {
      ...
      ensureMermaidRendered(this.source).then(() => {
        ...
        try {
          window.dispatchEvent(new CustomEvent('eidon:cm-relayout'));
        } catch {}
      });
    }
    return wrap;
        ...
        while (i <= lastLine) {
          ...
          if (imgMatch) {
            ...
            if (!cursorInside) {
              ...
              const isRelativeSrc = !/^(https?|data|blob|eidon-asset|file|\/\/|\/|[a-zA-Z]:)/i.test(rawSrc);
 ...
 * async-Mermaid `eidon:cm-relayout` window event into a field recompute.
 */
export function liveBlocksExtension(opts: BlockOptions = {}) {
  ...
  // fires `eidon:cm-relayout`. Translate that into a `relayoutEffect` so the
  ...
  const relayout = ViewPlugin.fromClass(
    class {
      ...
      constructor(view: EditorView) {
        ...
        window.addEventListener('eidon:cm-relayout', this.onRelayout);
      }
      destroy() {
        window.removeEventListener('eidon:cm-relayout', this.onRelayout);
      }
    },
  );
