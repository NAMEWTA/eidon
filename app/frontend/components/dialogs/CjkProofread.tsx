/**
 * CjkProofread.tsx — CJK 校对浮层。
 * ⌘⇧J 打开；列出 eidonInvoke('kn:cjkProofread') 的问题按严重度分组，点击跳转、Apply 替换、批量应用
...
export function CjkProofread({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function rescan() {
    ...
    setLoading(true);
    try {
      const result = await eidonInvoke('kn:cjkProofread', { text: tab.content ?? '' });
  ...
  function contextOf(issue: Issue): { before: string; hit: string; after: string } {
    ...
    const safeStart = Math.max(0, issue.colStart);
    const safeEnd = Math.min(bytes.length, issue.colEnd);
  ...
  function applyOne(issue: Issue) {
    ...
    const dec = new TextDecoder();
    const bytes = enc.encode(text);
    if (issue.colStart > bytes.length || issue.colEnd > bytes.length) {
      useToastsStore.getState().warning(t('proofread.outOfRange'));
      return;
    }
    const before = dec.decode(bytes.slice(0, issue.colStart));
    const after = dec.decode(bytes.slice(issue.colEnd));
  ...
  function applyAll(severity: 'high' | 'medium' | 'low' | 'all') {
    ...
    const sorted = target.slice().sort((a, b) => b.colStart - a.colStart);
    const enc = new TextEncoder();
    const dec = new TextDecoder();
    ...
    for (const issue of sorted) {
      if (issue.colEnd > lastStart) continue;
      if (issue.colStart > bytes.length || issue.colEnd > bytes.length) continue;
      const sugBytes = enc.encode(issue.suggestion);
      const merged = new Uint8Array(issue.colStart + sugBytes.length + (bytes.length - issue.colEnd));
      merged.set(bytes.slice(0, issue.colStart), 0);
      merged.set(sugBytes, issue.colStart);
      merged.set(bytes.slice(issue.colEnd), issue.colStart + sugBytes.length);
      bytes = merged;
      applied++;
      lastStart = issue.colStart;
    }
                ...
                <section key={bucket} className={`proof__bucket proof__bucket--${bucket}`}>
                  ...
                  <ul className="proof__list">
                    {grouped[bucket].map((issue, i) => {
                      ...
                      return (
                        <li
                          key={`${issue.colStart}-${issue.colEnd}-${i}`}
