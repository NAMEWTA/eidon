/**
 * SessionRestoreDialog.tsx — 跨设备会话恢复对话框。
 * 监听 `eidon:session-restore-available`；Restore 打开兄弟会话的标签并切换 active，
 * Keep mine 关闭。逻辑（relPath 解析、applySession）逐字保留。
 */
...
function resolveSessionTabPath(sib: { filePath: string | null; relPath?: string | null }): string | null {
  const root = useWorkspaceStore.getState().currentFolder || '';
  if (sib.relPath && root) {
    const sep = /^[a-zA-Z]:[\\/]/.test(root) ? '\\' : '/';
    const cleanRoot = root.replace(/[\\/]+$/, '');
    const cleanRel = sib.relPath.replace(/^[\\/]+/, '').replace(/[\\/]/g, sep);
...
async function applySession(payload: SessionPayload) {
  ...
  const target = payload.tabs[payload.activeIndex];
  if (target) {
...
export function SessionRestoreDialog() {
  ...
  async function restore() {
    ...
    setRestoring(true);
    try {
      const payload = await useCloudSyncStore.getState().loadSession(folder, sib.deviceId);
      if (!payload) {
      ...
      useToastsStore.getState().success(t('cloudSync.restoredToast', { device: sib.deviceLabel }));
    } catch (e) {
      ...
      <div className="srd" role="dialog" aria-modal="true">
        ...
        <p className="srd__lead">
          {t('cloudSync.restoreLead', {
            device: sibling?.deviceLabel ?? '?',
            ago: sibling ? timeAgoLabel(sibling.savedAt) : '',
            tabs: String(sibling?.tabCount ?? 0),
          })}
        </p>
