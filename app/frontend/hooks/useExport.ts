/**
 * useExport。HTML/DOCX/PDF/PDF-print/PNG 导出
...
import { writeText, writeHtml, writeImage } from '@bridge/ipc/clipboard';
import { createClipboardImage } from '@bridge/ipc/platform';
...
async function exportHtml() {
  ...
  try {
    await eidonInvoke('editor:writeFile', { path, content: html, encoding: 'UTF-8' });
...
async function exportDocx() {
  ...
  try {
    ...
    await eidonInvoke('editor:writeBinaryFile', { path, data: Array.from(buffer) });
...
async function exportPdf() {
  ...
  try {
    ...
    useToastsStore.getState().success('Exported to PDF');
  } catch (e) {
    console.error(e);
    useToastsStore.getState().dismiss(tid);
...
<div class="eidon-print-content preview-content">${body}</div>`;
  ...
  try {
    await eidonInvoke('editor:print', {});
  } catch (e) {
...
async function exportImage() {
  ...
  try {
    ...
    await eidonInvoke('editor:writeBinaryFile', { path, data: Array.from(buffer) });
    useToastsStore.getState().dismiss(tid);
    const msg = isSelection ? 'Exported selection to PNG image' : 'Exported to PNG image';
...
async function copyAsImage() {
  ...
  try {
    ...
    const img = createClipboardImage(bytes);
    ...
    try {
      ...
      if (path) {
        ...
        await eidonInvoke('editor:writeBinaryFile', { path, data: Array.from(buffer) });
        useToastsStore.getState().success('Clipboard failed — saved as PNG instead');
      } else {
