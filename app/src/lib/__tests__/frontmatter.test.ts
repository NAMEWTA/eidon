...
describe('UNIVERSAL_KEYS', () => {
  it('包含四个通用键', () => {
    expect([...UNIVERSAL_KEYS].sort()).toEqual(['created', 'tags', 'title', 'updated']);
  });
});
...
describe('ensureFrontmatterTimestamps', () => {
  ...
  it('有 frontmatter 但无 updated 字段时插入到 created 之后', () => {
    const raw = '---\ncreated: 2026-01-01 08:00:00\ntitle: X\n---\nbody';
    const out = ensureFrontmatterTimestamps(raw);
    const lines = out.split('\n');
    const createdIdx = lines.findIndex((l) => l.startsWith('created:'));
    expect(lines[createdIdx + 1]).toMatch(/^updated: /);
  });
  ...
  it('重复调用不累积字段、不破坏正文（注入后第二次只滚 updated）', () => {
    ...
    expect(twice.match(/updated:/g)).toHaveLength(1);
