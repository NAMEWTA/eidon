/**
 * useFiles — 文件操作（从 Vue composable 迁移为 React hook）。
 *
 * 纯命令式：所有动作经 `useXStore.getState()` 读写 store + core/bridge invoke，
 * 不依赖 React 渲染，故定义为模块级函数，hook 仅返回稳定句柄集合。行为逐字保留。
 *
 * 「未保存」对话框：Vue 版用 provide/inject + window 兜底；React 改为统一走
...
import { absoluteWorkspacePath, createWorkspaceFileStore } from '../../core/bridge/file';
...
async function nextAvailableInboxPath(workspaceFolder: string, inboxRelPath: string, fileName: string): Promise<string> {
  const store = createWorkspaceFileStore(workspaceFolder);
  const { stem, ext } = splitName(fileName);
  for (let i = 0; i < 1000; i++) {
    const name = i === 0 ? fileName : `${stem} ${i + 1}${ext}`;
    const candidate = joinRelPath(inboxRelPath, name);
...
async function createInDefaultInbox(fileName: string, language: 'markdown' | 'plaintext') {
  ...
  try {
    ...
    await invoke('fs_create_file', { path: targetPath, content: initialContent });
...
async function openAndConvert(path: string, ext: string) {
  ...
  try {
    ...
    // 转换为 markdown：直接以最终名/语言新建标签再灌内容（与 Vue 版「新建后 mutate」终态一致）。
...
/**
 * 保存前：md 文件一律保证 frontmatter 带 created/updated（缺则注入/补齐、updated 刷新为此刻），
 * 并把新内容同步进 store（markSaved 随后据此落 savedContent，标签页保持 clean）。非 md 原样返回。
 */
function syncBumpUpdated(tab: Tab): string {
  if (tab.language !== 'markdown') return tab.content;
  const bumped = ensureFrontmatterTimestamps(tab.content);
...
async function saveTab(tab: Tab, opts: { silent?: boolean } = {}): Promise<boolean> {
  ...
  try {
    ...
    await invoke('write_file', { path, content: payload, encoding: tab.encoding || 'UTF-8' });
    ...
    if (!opts.silent) {
      if (isIOS()) {
        ...
        toasts.success(`Saved to On My iPhone › EIDON › ${fname}`);
      } else {
...
async function saveTabAs(tab: Tab): Promise<boolean> {
  ...
  if (isIOS()) {
    ...
    path = await saveDialog({ defaultPath, filters: SAVE_FILTERS });
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent('eidon:saved', { detail: { filePath: path } }));
    const fileName = path.split(/[\\/]/).pop() ?? path;
    toasts.success(isIOS() ? `Saved to On My iPhone › EIDON › ${fileName}` : `Saved as ${fileName}`);
    return true;
  } catch (e) {
