/**
 * HistoryPanel.tsx — AutoGit 每文档历史面板（从 HistoryPanel.vue 迁移）。
...
export function HistoryPanel({ onClose }: { onClose?: () => void }) {
  ...
  const headSha = useGitHistoryStore((s) => s.status?.head_sha);
        ...
        <ul className="history__list">
          {commits.map((c) => (
            <li key={c.sha} className="history__item">
              ...
              >
                <span className="history__sha">{c.short_sha}</span>
              ...
              {expandedSha === c.sha && (
                <div className="history__diff-wrap">
                  <div className="history__diff-toolbar">
                    <button className="history__restore" onClick={() => onRestore(c.sha, c.short_sha)}>
                      {t('history.restore')}
                    </button>
