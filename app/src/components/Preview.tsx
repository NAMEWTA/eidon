/**
 * Preview.tsx — markdown 渲染面命令式包装器（从 Preview.vue 迁移）。
 *
...
export interface PreviewProps {
  ...
  /** 相对路径链接的打开回调（Vue 版直接调 files.openPath；由 App/PaneContent 注入）。 */
...
export const Preview = forwardRef<PreviewHandle, PreviewProps>(function Preview(props, ref) {
  ...
  // 切换当前笔记后会用旧目录解析相对路径（复刻 Vue 每次读响应式 props.filePath 的行为）。
